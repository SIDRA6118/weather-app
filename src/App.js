...
const API_KEY = process.env.REACT_APP_OPENWEATHER_KEY; // set on Vercel
...
function buildDaily(list) {
  ...
  const days = [...map.entries()]
    ...
    .map(([date, items]) => {
      ...
      items.forEach((i) => {
        if (
          Math.abs(new Date(i.dt_txt).getHours() - 12) <
          Math.abs(new Date(best.dt_txt).getHours() - 12)
        ) {
          best = i;
        }
      });
...
export default function App() {
  ...
  async function fetchWeather(city, unit) {
    if (!API_KEY) {
      setErrorMsg("Missing OpenWeather API key in .env");
      return;
    }
    if (!city) return;
    setLoading(true);
  ...
  return (
    <div className="container">
      <form className="search-row" onSubmit={onSubmit}>
        ...
        <button className="search-btn" type="submit" title="Search">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path
              d="M21 21l-4.2-4.2m1.4-5.3a7 7 0 11-14 0 7 7 0 0114 0z"
              stroke="black"
              strokeWidth="2"
              strokeLinecap="round"
            />
          ...
          <div className="metrics">
            <div className="metric" title="Wind speed">
              <svg width="20" height="20" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  d="M4 12h11a3 3 0 100-6 3 3 0 00-3 3"
                  fill="none"
                  stroke="#000"
                  strokeWidth="2"
                  strokeLinecap="round"
              ...
              <div>
                ...
                <div>{now.wind.speed.toFixed(2)} {windUnit}</div>
              </div>
            </div>
            <div className="metric" title="Humidity">
              <svg width="20" height="20" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  d="M12 3s6 6.4 6 10a6 6 0 11-12 0c0-3.6 6-10 6-10z"
                  fill="none"
                  stroke="#000"
                  strokeWidth="2"
                />
      ...
      {loading && (
        <div className="subdued" style={{ textAlign: "center" }}>
          Loading…
        </div>
      )}
      {errorMsg && (
        <div className="subdued" style={{ color: "#b91c1c", textAlign: "center" }}>
          {errorMsg}
        </div>
      )}

      <div className="footer">Coded by You · Open sourced on GitHub · Hosted on Vercel</div>
    </div>
  );
}
